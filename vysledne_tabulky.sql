...
WITH payroll_filtered AS (
    SELECT
      cp.payroll_year::int AS year,
      cp.industry_branch_code::text AS industry_branch_code,
      ib.name AS industry_branch_name,
      cp.calculation_code::int AS calculation_code,
      cp.value::numeric AS wage_value
    FROM czechia_payroll cp
    JOIN czechia_payroll_industry_branch ib
      ON ib.code::text = cp.industry_branch_code::text
    WHERE cp.value IS NOT NULL
      AND cp.industry_branch_code IS NOT NULL
      AND cp.value_type_code = 5958
      AND cp.unit_code = 200
),
wage_by_branch_calc AS (
    SELECT
      year,
      industry_branch_code,
      industry_branch_name,
      calculation_code,
      AVG(wage_value) AS avg_wage_czk
    FROM payroll_filtered
    GROUP BY year, industry_branch_code, industry_branch_name, calculation_code
),
wage_by_branch AS (
    SELECT
      year,
      ...
      industry_branch_name,
      COALESCE(
        MAX(avg_wage_czk) FILTER (WHERE calculation_code = 200),
        MAX(avg_wage_czk) FILTER (WHERE calculation_code = 100),
        MAX(avg_wage_czk)
      ) AS avg_wage_czk
    FROM wage_by_branch_calc
    GROUP BY year, industry_branch_code, industry_branch_name
),
wage_all AS (
    SELECT
      year,
...
wages AS (
    SELECT * FROM wage_by_branch
    UNION ALL
    SELECT * FROM wage_all
),
prices AS (
    SELECT
